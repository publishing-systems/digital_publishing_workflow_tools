 ...
 */
/**
 * @file $/text_concatenator/text_concatenator_1/text_concatenator_1.java
 * @brief A text concatenator that can also just concatenate specific portions of
 *     several input files.
...
public class text_concatenator_1
{
    public static void main(String args[])
    {
        System.out.print("text_concatenator_1 Copyright (C) 2016-2018 Stephan Kreutzer\n" +
        ...
        text_concatenator_1 concatenator = new text_concatenator_1();

        try
        {
            concatenator.execute(args);
        }
        catch (ProgramTerminationException ex)
        {
            concatenator.handleTermination(ex);
        }

        if (concatenator.resultInfoFile != null)
        {
            try
            {
                BufferedWriter writer = new BufferedWriter(
                                        new OutputStreamWriter(
                                        new FileOutputStream(concatenator.resultInfoFile),
                                        "UTF-8"));

                writer.write("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
                writer.write("<!-- This file was created by text_concatenator_1, which is free software licensed under the GNU Affero General Public License 3 or any later version (see https://github.com/publishing-systems/digital_publishing_workflow_tools/ and http://www.publishing-systems.org). -->\n");
                writer.write("<text-concatenator-1-result-information>\n");
                writer.write("  <success>\n");

                if (concatenator.getInfoMessages().size() > 0)
                {
                    writer.write("    <info-messages>\n");

                    for (int i = 0, max = concatenator.getInfoMessages().size(); i < max; i++)
                    {
                        InfoMessage infoMessage = concatenator.getInfoMessages().get(i);
    ...
    public int execute(String args[])
    {
        ...
        text_concatenator_1.resultInfoFile = resultInfoFile;
        ...
        System.out.println("text_concatenator_1: " + getI10nStringFormatted("messageCallDetails", jobFile.getAbsolutePath(), resultInfoFile.getAbsolutePath()));
        ...
        try
        {
            ...
            while (eventReader.hasNext() == true)
            {
                ...
                if (event.isStartElement() == true)
                {
                    ...
                    if (tagName.equals("text-file") == true)
                    {
                        StartElement textFileElement = event.asStartElement();
                        Attribute pathAttribute = textFileElement.getAttributeByName(new QName("path"));
                        Attribute startAttribute = textFileElement.getAttributeByName(new QName("start"));
                        Attribute lengthAttribute = textFileElement.getAttributeByName(new QName("length"));
        ...
        {
            throw constructTermination("messageJobFileNoInputFilesSpecified", null, null, jobFile.getAbsolutePath(), "input-file");
        }
                                            ...
                                            Object ... arguments)
    {
        if (message == null)
        {
            if (arguments == null)
            {
                message = "text_concatenator_1: " + getI10nString(id);
            }
            else
            {
                message = "text_concatenator_1: " + getI10nStringFormatted(id, arguments);
            }
        }
    ...
    public int handleTermination(ProgramTerminationException ex)
    {
        ...
        if (text_concatenator_1.resultInfoFile != null)
        {
            try
            {
                BufferedWriter writer = new BufferedWriter(
                                        new OutputStreamWriter(
                                        new FileOutputStream(text_concatenator_1.resultInfoFile),
                                        "UTF-8"));
        ...
        text_concatenator_1.resultInfoFile = null;
    ...
    private static final String L10N_BUNDLE = "l10n.l10nTextConcatenator1Console";
